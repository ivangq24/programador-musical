...
export default function SimpleMantenimientoCanciones() {
  ...
  // Datos de prueba
  const mockCanciones = [
    { id: 1, titulo: "Test Song", artista: "Test Artist", album: "Test Album", duracion: 180, categoria_id: null, activa: true },
    { id: 2, titulo: "Nueva Canción", artista: "Nuevo Artista", album: "Nuevo Álbum", duracion: 200, categoria_id: null, activa: true },
    { id: 3, titulo: "Canción Final", artista: "Artista Final", album: "Álbum Final", duracion: 300, categoria_id: null, activa: true },
    { id: 4, titulo: "Canción Test Final", artista: "Artista Test Final", album: "Álbum Test Final", duracion: 250, categoria_id: null, activa: true },
    { id: 5, titulo: "Canción Final Test", artista: "Artista Final Test", album: "Álbum Final Test", duracion: 300, categoria_id: null, activa: true },
    { id: 6, titulo: "Test Song Backend", artista: "Test Artist Backend", album: "Test Album", duracion: 180, categoria_id: null, activa: true },
    { id: 7, titulo: "Canción Verificada", artista: "Artista Verificado", album: "Álbum Verificado", duracion: 240, categoria_id: null, activa: true }
  ]
  ...
  const handleSaveCancion = async (cancionData) => {
    try {
      setLoading(true)
      ...
      if (cancionEditando) {
        ...
        console.log('Canción actualizada:', cancionData)
      } else {
        // Crear nueva canción
        await createCancion(cancionData)
        console.log('Canción creada:', cancionData)
      }
      ...
      console.error('Error saving cancion:', error)
  ...
  const handleDeleteCancion = async (cancionId) => {
    const cancion = canciones.find(c => c.id === cancionId)
    if (cancion) {
      if (confirm(`¿Estás seguro de que quieres eliminar "${cancion.titulo}"?`)) {
        try {
          setLoading(true)
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              <button 
                ...
                className="flex items-center space-x-2 px-4 py-2 rounded-lg bg-green-500 hover:bg-green-600 text-white transition-all duration-200 shadow-md hover:shadow-lg transform hover:-translate-y-0.5"
              >
                ...
                filteredCanciones.map((cancion, index) => (
                  <tr 
                    ...
                    className={`hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 transition-all duration-200 cursor-pointer group ${
                      selectedCanciones.includes(cancion.id) 
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center space-x-2">
                        ...
                        <button 
                          onClick={() => handleMoreOptions(cancion.id)}
